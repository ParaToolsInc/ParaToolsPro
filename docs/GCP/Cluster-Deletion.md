...
destroy it. When a cluster is created, `gcluster` provisions infrastructure (networks,
subnets, NAT gateways, IP addresses, filestore instances, Slurm controller/login VMs,
etc.); if any of these are deleted out-of-band, some will remain and you will be
charged for them. To delete your cluster correctly run, from the same directory you
ran `gcluster create` from:
```
./gcluster destroy DEPLOYMENT_NAME/
```
where `DEPLOYMENT_NAME` is the value of `vars.deployment_name:` from your blueprint
(in the example tutorial: `ppro-e4s-25-11-cluster`).

## Improper Deletion
### Case 1
When the compute instances are deleted but the deployment folder still exists, you can
run the command `./gcluster destroy DEPLOYMENT_NAME/` and it should properly remove all
remaining created resources. You should also run `rm -rf DEPLOYMENT_NAME/` to remove
the deployment folder.
### Case 2
When the deployment folder hasn't been deleted, and you attempt to create the cluster
again, you may get the error
```
...
```
In this case either remove the folder as stated above, or pass the `-w` flag to
`gcluster create` to overwrite the existing deployment folder with the new contents.
### Case 3 -- orphaned infrastructure
If you are getting errors like the one below, it indicates `gcluster` is unable to
recreate a cluster due to leftover resources from a prior deployment that was not
cleanly destroyed.
```
...
```
You must manually delete the leftover resources before `gcluster` can recreate them.
The most common categories are:

- **Cloud NAT external IP addresses** -- shown directly in the error above. Find them
  under __VPC network > IP addresses__ (or `gcloud compute addresses list`) and release them.
- **Filestore instances** (the home filesystem). Find them under __Filestore > Instances__.
  Be very careful: confirm by creation date and name that the instance belongs to the
  cluster you are deleting, since other clusters may share filestore namespace.
- **VPC network resources** -- VPC network, subnetworks, Cloud Router, NAT gateway, peering.
  These often must be deleted in a specific order: NAT gateway first, then subnetwork,
  then VPC network peering, then Cloud Router, then VPC, then release the IP addresses.
  If a resource will not delete because "in use by another," find and delete the
  resource holding it first.

After all stray resources are removed, run `./gcluster create -w DEPLOYMENT_NAME/` (or
re-run `gcluster create` against the original blueprint), and then `./gcluster deploy
DEPLOYMENT_NAME/`. If new "already exists" errors appear, repeat the cleanup above.
